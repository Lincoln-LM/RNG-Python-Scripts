...
def find(lottos,swarm=None,sphere1=None,sphere2=None,print_=True):
    ...
    while lottos[0] < 0x100000:
        for low in range(0x1000):
            ...
            seed_q = LCRNG.MRNGR(seed_q).next()
            daily_q = seed_q
            ...
            if sphere2 != None:
                if (daily_q%1000)//200 != sphere2:
                    continue
            flag = True
            temp = LCRNG.ARNG(seed_q).next()
            for lotto in lottos[1:]:
                flag2 = False
                while lotto < 0x100000:
                    test = (LCRNG.MRNG(temp).next()>>0xc)
                    if test == lotto:
                        flag2 = True
                        break
                    lotto += 100000
...
def main():
    ...
    temp = seed
    for _ in range(2):
        lottos.append((LCRNG.MRNG(temp).next()>>0xc)%100000)
        temp = LCRNG.ARNG(temp).next()
...
def test(tests):
    for t in range(tests):
        ...
        print(swarm)
        for _ in range(2):
            lotto = (LCRNG.MRNG(temp).next()>>0xc)%100000
            lottos.append(lotto)
            print(lotto)
            temp = LCRNG.ARNG(temp).next()
